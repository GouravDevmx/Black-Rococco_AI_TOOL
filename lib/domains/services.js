...
async function handleAdminRoutes({ req, res, pathname, db, salonId }) {
  if (req.method === 'POST' && pathname === '/api/admin/services') {
    ...
    const service = {
      ...
      area: ['hands','feet','both'].includes(body.area) ? body.area : 'hands',
  ...
  if (serviceMatch) {
    ...
    if (req.method === 'PATCH') {
      ...
      if (body.area !== undefined) service.area = ['hands','feet','both'].includes(body.area) ? body.area : service.area || 'hands';
