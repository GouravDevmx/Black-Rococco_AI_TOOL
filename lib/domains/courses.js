...
async function handlePublicRoutes({ req, res, pathname, db, salonId }) {
  if (req.method === 'POST' && pathname === '/api/course-registrations') {
    ...
    if (verify.REQUIRE_BOOKING_VERIFICATION) {
      const session = currentClientSession(req);
      if (!session && !verify.isVerifiedForWhatsapp(req, body.whatsapp)) {
        return json(res, 403, { error: 'Verifica tu WhatsApp para confirmar tu inscripción.', needVerification: true }), true;
      }
    }
...
async function handleAdminRoutes({ req, res, pathname, db, salonId }) {
  ...
  if (req.method === 'PATCH' && registrationMatch) {
    ...
    if (!['new', 'in_progress', 'pending_payment', 'confirmed', 'closed', 'deleted'].includes(next)) { json(res, 400, { error: 'Estado inválido.' }); return true; }
