...
/* Validate an inline chat image: must be a data URL of an allowed image
   type and under ~1.6MB of base64 (≈1.2MB binary). Returns the cleaned
   value or null. Strict prefix check prevents javascript:/html injection. */
const CHAT_IMAGE_RE = /^data:image\/(png|jpeg|jpg|webp|gif);base64,[A-Za-z0-9+/=]+$/;
const CHAT_IMAGE_MAX_LEN = 950_000; // fits inside readBody's 1MB cap
function safeChatImage(value) {
  if (typeof value !== 'string' || !value) return null;
  if (value.length > CHAT_IMAGE_MAX_LEN) return null;
  return CHAT_IMAGE_RE.test(value) ? value : null;
}

async function handlePublicRoutes({ req, res, pathname, db, salonId }) {
  // POST /api/chat/messages — visitor sends a message (text and/or image)
  if (req.method === 'POST' && pathname === '/api/chat/messages') {
    const body = await readBody(req);
    const text = safeString(body.text, 2000).trim();
    const imageUrl = safeChatImage(body.image);
    if (!text && !imageUrl) return json(res, 400, { error: 'Escribe un mensaje.' }), true;
    ...
    const msg = {
      ...
      whatsapp: safeString(body.whatsapp, 30) || '',
      text,
...
async function handleAdminRoutes({ req, res, pathname, db, salonId }) {
  ...
  if (req.method === 'GET' && pathname === '/api/admin/chats') {
    ...
    const threads = [...byThread.entries()].map(([threadId, msgs]) => {
      ...
      return {
        ...
        whatsapp: msgs.find(m => m.sender === 'client' && m.whatsapp)?.whatsapp || '',
        lastText: last.text || (last.imageUrl ? '📷 Imagen' : ''),
  ...
  // POST /api/admin/chats/:threadId/reply — text and/or image
