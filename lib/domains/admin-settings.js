...
async function handleAdminRoutes({ req, res, pathname, db, salonId }) {
  // PUT /api/admin/settings/reminders — timing + wording, editable without a deploy
  if (req.method === 'PUT' && pathname === '/api/admin/settings/reminders') {
    const body = await readBody(req);
    const r = body.reminders || {};
    const clampTpl = v => safeString(v, 1200);
    const cur = db.settings.reminders || {};
    db.settings.reminders = {
      enabled: r.enabled !== false,
      onBooking: {
        enabled: r.onBooking?.enabled !== false,
        template: clampTpl(r.onBooking?.template) || cur.onBooking?.template || ''
      },
      // Cap the schedule so a bad client can't create hundreds of send rules.
      schedule: (Array.isArray(r.schedule) ? r.schedule : []).slice(0, 12)
        .map(s => ({
          hoursBefore: Math.min(336, Math.max(1, Math.round(Number(s.hoursBefore) || 24))),
          enabled: s.enabled !== false,
          template: clampTpl(s.template)
        }))
        .filter(s => s.template),
      afterVisit: {
        enabled: r.afterVisit?.enabled === true,
        hoursAfter: Math.min(168, Math.max(1, Math.round(Number(r.afterVisit?.hoursAfter) || 3))),
        template: clampTpl(r.afterVisit?.template) || cur.afterVisit?.template || ''
      }
    };
    await writeDb(db, salonId);
    json(res, 200, { ok: true, reminders: db.settings.reminders });
    return true;
  }
  ...
  if (req.method === 'POST' && pathname === '/api/admin/settings/brand') {
    ...
    db.settings.brand = {
      ...
      ctas: (() => {
        const cur = brand.ctas || {};
        const inc = body.ctas;
        if (!inc || typeof inc !== 'object') return cur;
        return {
          density: ['minimal', 'balanced', 'high'].includes(inc.density) ? inc.density : 'balanced',
          label: safeString(inc.label, 40) || cur.label || 'RESERVAR MI CITA',
          note: safeString(inc.note, 120)
        };
      })(),
      priceAnchor: (() => {
        const cur = brand.priceAnchor || {};
        const inc = body.priceAnchor;
        if (!inc || typeof inc !== 'object') return cur;
        return {
          enabled: inc.enabled !== false,
          mode: inc.mode === 'manual' ? 'manual' : 'auto',
          manualPrice: Math.max(0, Math.round(Number(inc.manualPrice) || 0)),
          label: safeString(inc.label, 40) || cur.label || 'Desde',
          note: safeString(inc.note, 80)
        };
      })()
    };
    await writeDb(db, salonId);
