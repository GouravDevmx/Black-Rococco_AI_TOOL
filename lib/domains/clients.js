...
function applyClientProfilePatch(client, body = {}, { allowIdentity = true, onlyNonEmpty = false } = {}) {
  ...
  setText('notes', 1200);
  // Deposit-on-file flag: only meaningful from the admin CRM (allowIdentity
  // is true for admin patches, false for client self-service).
  if (allowIdentity && body.depositOnFile !== undefined) client.depositOnFile = body.depositOnFile === true;
...
function clientWithStats(db, client) {
  ...
  const activeAppointments = allAppointments.filter(a => !['cancelled', 'deleted'].includes(a.status));
  const now = Date.now();
  const upcoming = activeAppointments
    .filter(a => !['completed', 'closed', 'cancelled', 'deleted'].includes(a.status) && appointmentDateTime(a).getTime() >= now)
  ...
  return {
    ...
    cancelledVisits: allAppointments.filter(a => ['cancelled', 'deleted'].includes(a.status)).length,
    lastVisit,
