...
const { buildMessage, confirmUrl } = require('./reminders');
...
async function processClientReminders(salonId) {
  ...
  try {
    ...
    db = await readDb(salonId, ['appointments', 'services', 'clients', 'notifications', 'staff']);
  } catch (_) {
    return;
  }
  const now = Date.now();
  for (const appt of db.appointments) {
    ...
    // Schedule comes from admin-editable settings so the salon can retime
    // reminders without a redeploy; env vars remain the fresh-install default.
    const cfg = db.settings?.reminders;
    const scheduleHours = (cfg && cfg.enabled !== false && Array.isArray(cfg.schedule))
      ? cfg.schedule.filter(s => s.enabled !== false).map(s => Number(s.hoursBefore)).filter(Boolean)
      : CLIENT_REMINDER_HOURS;
    for (const hoursBefore of scheduleHours) {
      ...
      // Ship the ready-to-send text so the automation (n8n/Make) just forwards
      // it, instead of every integration re-inventing the wording. Includes the
      // one-tap confirm link so the client can respond without replying.
      const rendered = buildMessage(db, appt, hoursBefore);
      if (rendered) {
        payload.message = rendered;
        payload.confirmUrl = confirmUrl(db, appt);
      }
