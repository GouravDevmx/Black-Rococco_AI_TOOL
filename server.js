...
async function handleApi(req, res, pathname, url) {
  try {
    ...
    if (req.method === 'GET' && pathname === '/api/admin/google-calendar/callback') {
      return googleCalendarDomain.handleCallbackRoute(req, res, url, salonRef.get() || SALON_ID);
    }

    const salonId = salonRef.get() || SALON_ID;
    const salon = SALON;
...
const server = http.createServer((req, res) => {
  ...
  try {
    ...
    if (/^\/(blog|academia)\/[^/]+\/?$/.test(pathname) || pathname === '/sitemap.xml') {
      return seoDomain.handleSeoRoutes({ req, res, pathname, readDb, salonId: salonRef.get() || SALON_ID })
...
async function startServer() {
  ...
  const runReminders = () => notificationsDomain
    .processClientReminders(salonRef.get() || SALON_ID)
