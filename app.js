...
async function createBooking() {
  // Guard: prevent double-tap while the booking is in flight.
  if (state.booking._submitting) return;
  state.booking._submitting = true;
  state.booking.error = '';
  render();
  ...
  try {
    ...
    // Slot conflict: push the client back to Step 2 to pick a new time,
    // rather than showing a vague error on the confirm screen.
    if (/ocuparse|conflicto|no.*disponible|slot.*taken/i.test(msg)) {
      state.booking.step = 2;
      state.booking.time = null;
      state.booking.holdError = msg || 'Ese horario acaba de ocuparse. Elige otro.';
      await loadAvailability();
    } else {
      state.booking.error = msg;
    }
  } finally {
    state.booking._submitting = false;
  }
  render();
}
  ...
  return `<div class="booking-step">
    ...
    ${b.loadingSlots ? `<div class="empty">Cargando horarios…</div>` : freeSlots.length
      ? `<div class="slot-count">${freeSlots.length} horario${freeSlots.length !== 1 ? 's' : ''} disponible${freeSlots.length !== 1 ? 's' : ''}</div>
         <div class="time-grid">${freeSlots.map(s => `<button class="time-btn${b.time === s.time ? ' active' : ''}" ${b._holding ? 'disabled' : ''} data-time="${s.time}"><span>${s.time}</span></button>`).join('')}</div>`
...
function bookingStepConfirm() {
  ...
  return `<div class="booking-step">
    ...
    ${b.error ? `<div class="error-box">${esc(b.error)}</div>` : ''}
    <button class="btn btn-primary" data-confirm-booking ${b._submitting ? 'disabled' : ''}>${b._submitting ? 'CONFIRMANDO…' : 'CONFIRMAR CITA'}</button>
  </div>`;
}
...
app.addEventListener('click', async event => {
  ...
  if (target.dataset.time) {
    if (state.booking._holding) return; // prevent double-tap while hold is in flight
    state.booking.time = target.dataset.time;
    state.booking.holdError = '';
    state.booking._holding = true;
    render(); // show the selected time immediately (visual feedback)
    const held = await holdSelectedSlot();
    state.booking._holding = false;
    if (held) {
...
function connectRealtime() {
  ...
  es.addEventListener('availability', e => {
    ...
    if (state.tab !== 'reservar') return;
    if (info.date && info.date !== state.booking.date) return;

    // Step 2: refresh the time grid immediately.
    if (state.booking.step === 2) {
      loadAvailability();
      return;
    }

    // Step 3: the client is filling the confirm form. Re-check if their held
    // slot is still valid. If someone else booked the slot (hold expired and
    // they grabbed it), warn the client BEFORE they tap CONFIRMAR.
    if (state.booking.step === 3 && state.booking.time) {
      (async () => {
        try {
          const ids = (state.booking.serviceIds || []).filter(Boolean);
          const q = ids.length > 1
            ? `serviceIds=${encodeURIComponent(ids.join(','))}`
            : `serviceId=${encodeURIComponent(state.booking.serviceId)}`;
          const data = await api(`/api/availability?${q}&date=${encodeURIComponent(state.booking.date)}&holder=${encodeURIComponent(holderId())}`);
          const mySlot = (data.slots || []).find(s => s.time === state.booking.time);
          if (mySlot && mySlot.busy) {
            // Slot was taken while the client was filling the form. Push back
            // to Step 2 with a clear message instead of letting them submit
            // and get a confusing server error.
            state.booking.step = 2;
            state.booking.time = null;
            state.booking.holdError = 'Tu horario fue tomado por otra clienta. Elige otro.';
            state.booking.slots = data.slots;
            render();
          }
        } catch (_) { /* network blip — server validates at booking time anyway */ }
      })();
    }
  });
